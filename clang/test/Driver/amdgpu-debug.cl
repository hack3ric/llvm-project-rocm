...
// RUN: %clang -### -target amdgcn-amd-amdhsa -x cl -nogpulib -c -emit-llvm -g %s 2>&1 | FileCheck -check-prefix=CHECK-SIMPLE %s
// RUN: %clang -### -target amdgcn-amd-amdhsa -x cl -nogpulib -c -emit-llvm -ggdb %s 2>&1 | FileCheck -check-prefix=CHECK-SIMPLE %s
// CHECK-SIMPLE: "-cc1"
...
// RUN: %clang -### -target amdgcn-amd-amdhsa -x cl -nogpulib -c -emit-llvm %s 2>&1 | FileCheck -check-prefix=CHECK-NO-G %s
// CHECK-NO-G: "-cc1"
...
// RUN: %clang -### -target amdgcn-amd-amdhsa -x cl -nogpulib -c -emit-llvm -g -gno-heterogeneous-dwarf %s 2>&1 | FileCheck -check-prefix=CHECK-NO-HETEROGENEOUS %s
