...
TEST_F(DIExprBuilderTest, NonUnique) {
  DIExprBuilder BuilderA(Context);
  BuilderA.append<DIOp::Referrer>(Type::getInt32Ty(Context));
  DIExprBuilder BuilderB(Context);
  BuilderB.append<DIOp::Referrer>(Type::getInt32Ty(Context));
...
TEST_F(DIExprBuilderTest, Unique) {
  ...
  BuilderA.append<DIOp::Referrer>(Type::getInt32Ty(Context));
  DIExprBuilder BuilderB(Context);
  BuilderB.append<DIOp::Referrer>(Type::getInt64Ty(Context));
  EXPECT_NE(BuilderA.intoExpr(), BuilderB.intoExpr());
}

TEST_F(DIExprBuilderTest, EquivalentAppends) {
  DIExprBuilder BuilderA(Context);
  BuilderA.append<DIOp::Referrer>(Type::getInt64Ty(Context));
  DIExprBuilder BuilderB(Context);
  BuilderB.append(DIOp::Referrer(Type::getInt64Ty(Context)));
  DIExprBuilder BuilderC(Context);
  BuilderC.append(DIOp::Variant(DIOp::Referrer(Type::getInt64Ty(Context))));
  DIExpr *ExprA = BuilderA.intoExpr();
...
TEST_F(DIExprBuilderTest, Iterator) {
  ...
  DIOp::Variant Op(std::in_place_type<DIOp::Referrer>,
...
TEST_F(DIExprBuilderTest, IteratorRange) {
  SmallVector<DIOp::Variant> Ops{DIOp::Variant{std::in_place_type<DIOp::Arg>, 0,
                                               Type::getInt64Ty(Context)},
                                 DIOp::Variant{std::in_place_type<DIOp::Arg>, 1,
                                               Type::getInt64Ty(Context)},
...
TEST_F(DIExprBuilderTest, InitializerList) {
  std::initializer_list<DIOp::Variant> IL{
      DIOp::Variant{std::in_place_type<DIOp::Arg>, 0,
                    Type::getInt64Ty(Context)},
      DIOp::Variant{std::in_place_type<DIOp::Arg>, 1,
                    Type::getInt64Ty(Context)},
      DIOp::Variant{std::in_place_type<DIOp::Add>}};
...
TEST_F(DIExprBuilderTest, Erase) {
  DIExprBuilder BuilderA(Context,
                         {DIOp::Variant{std::in_place_type<DIOp::Referrer>,
                                        Type::getInt64Ty(Context)},
                          DIOp::Variant{std::in_place_type<DIOp::Arg>, 0,
                                        Type::getInt64Ty(Context)},
                          DIOp::Variant{std::in_place_type<DIOp::Add>},
...
typedef MetadataTest DIExprViewerTest;

TEST_F(DIExprViewerTest, Iterator) {
  DIExprBuilder Builder(Context);
  DIOp::Variant Op(std::in_place_type<DIOp::Referrer>,
                   Type::getInt32Ty(Context));
  DIExpr *Expr = Builder.append(Op).intoExpr();
  DIExprViewer ViewViewer = Expr->viewer();
  DIExprViewer::Iterator I = ViewViewer.begin();
  DIExprViewer::Iterator E = ViewViewer.end();
  EXPECT_EQ(*I, Op);
  ++I;
  EXPECT_EQ(I, E);
  --I;
...
TEST_F(DIExprOpsTest, Referrer) {
  DIOp::Variant V{std::in_place_type<DIOp::Referrer>,
                  Type::getInt64Ty(Context)};
  ASSERT_TRUE(std::holds_alternative<DIOp::Referrer>(V));
  ASSERT_EQ(std::get<DIOp::Referrer>(V).getResultType(),
            Type::getInt64Ty(Context));
}

TEST_F(DIExprOpsTest, Arg) {
  DIOp::Variant V{std::in_place_type<DIOp::Arg>, 1u, Type::getInt64Ty(Context)};
  ASSERT_TRUE(std::holds_alternative<DIOp::Arg>(V));
  ASSERT_EQ(std::get<DIOp::Arg>(V).getIndex(), 1u);
  ASSERT_EQ(std::get<DIOp::Arg>(V).getResultType(), Type::getInt64Ty(Context));
}

TEST_F(DIExprOpsTest, TypeObject) {
  DIOp::Variant V{std::in_place_type<DIOp::TypeObject>,
                  Type::getInt64Ty(Context)};
  ASSERT_TRUE(std::holds_alternative<DIOp::TypeObject>(V));
  ASSERT_EQ(std::get<DIOp::TypeObject>(V).getResultType(),
            Type::getInt64Ty(Context));
}
...
TEST_F(DIExprOpsTest, Convert) {
  DIOp::Variant V{std::in_place_type<DIOp::Convert>, Type::getInt64Ty(Context)};
  ASSERT_TRUE(std::holds_alternative<DIOp::Convert>(V));
  ASSERT_EQ(std::get<DIOp::Convert>(V).getResultType(),
            Type::getInt64Ty(Context));
}

TEST_F(DIExprOpsTest, Reinterpret) {
  DIOp::Variant V{std::in_place_type<DIOp::Reinterpret>,
                  Type::getInt64Ty(Context)};
  ASSERT_TRUE(std::holds_alternative<DIOp::Reinterpret>(V));
  ASSERT_EQ(std::get<DIOp::Reinterpret>(V).getResultType(),
            Type::getInt64Ty(Context));
}

TEST_F(DIExprOpsTest, BitOffset) {
  DIOp::Variant V{std::in_place_type<DIOp::BitOffset>,
                  Type::getInt64Ty(Context)};
  ASSERT_TRUE(std::holds_alternative<DIOp::BitOffset>(V));
  ASSERT_EQ(std::get<DIOp::BitOffset>(V).getResultType(),
            Type::getInt64Ty(Context));
}

TEST_F(DIExprOpsTest, ByteOffset) {
  DIOp::Variant V{std::in_place_type<DIOp::ByteOffset>,
                  Type::getInt64Ty(Context)};
  ASSERT_TRUE(std::holds_alternative<DIOp::ByteOffset>(V));
  ASSERT_EQ(std::get<DIOp::ByteOffset>(V).getResultType(),
            Type::getInt64Ty(Context));
}

TEST_F(DIExprOpsTest, Composite) {
  DIOp::Variant V{std::in_place_type<DIOp::Composite>, 4u,
                  Type::getInt64Ty(Context)};
  ASSERT_TRUE(std::holds_alternative<DIOp::Composite>(V));
  ASSERT_EQ(std::get<DIOp::Composite>(V).getCount(), 4u);
  ASSERT_EQ(std::get<DIOp::Composite>(V).getResultType(),
            Type::getInt64Ty(Context));
}
...
TEST_F(DIExprOpsTest, Deref) {
  DIOp::Variant V{std::in_place_type<DIOp::Deref>, Type::getInt64Ty(Context)};
  ASSERT_TRUE(std::holds_alternative<DIOp::Deref>(V));
  ASSERT_EQ(std::get<DIOp::Deref>(V).getResultType(),
            Type::getInt64Ty(Context));
}
...
TEST_F(DIExprOpsTest, PushLane) {
  DIOp::Variant V{std::in_place_type<DIOp::PushLane>,
                  Type::getInt64Ty(Context)};
  ASSERT_TRUE(std::holds_alternative<DIOp::PushLane>(V));
  ASSERT_EQ(std::get<DIOp::PushLane>(V).getResultType(),
            Type::getInt64Ty(Context));
}
